...
boxplot_payments <- function(data, payments){
  DRG <- data %>%
    mutate(def = sub("-.*", "", DRG.Definition)) ## mutate the DRG.Definition to first 3 numbers

  ggplot(DRG, aes(x = def, y = get(payments))) + ## initial the plots
    geom_boxplot() + ## make boxplot
    theme(axis.text.x = element_text(angle = 90))+ ## tilt x-axis labels 
    labs(
      x = 'Codes', ## relabel x axis
      y = "Average Payments", ## relabel y axis
      title = str_to_title(gsub("[.]", " ", str_to_title(payments)))) ## name the title of the plot
}
#' DRG statistics
#'
#' @param data a dataframe
